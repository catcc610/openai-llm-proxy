...
class ModelTester:
    ...
    def __init__(self, base_url: str = "http://localhost:9000/v1", api_key: str = "dummy"):
        """初始化测试器"""
        ...
        # 测试用的图片
        # 1. 真实图片URL
        self.test_image_url = "https://mag.npf.co.jp/wp-content/uploads/2023/10/27750021_m.jpg"
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ 加载配置文件成功，找到 {len(config.get('model_config', {}))} 个模型")
        return config
    ...
    async def test_multimodal(self, model: str) -> TestResult:
        ...
        try:
            ...
            combined_preview = f"URL图片: {content1[:80]}... | Base64图片: {content2[:80]}..."
    ...
    def print_results(self, results: List[TestResult]) -> None:
        ...
        for model, model_results in models.items():
            ...
            for result in model_results:
                ...
                print(f"   {status} {result.test_type:<12} | "
    ...
    def save_results_json(self, results: List[TestResult], filename: str = "test_results.json") -> None:
        """保存测试结果为JSON文件"""
        results_data = []
        for result in results:
            results_data.append({
